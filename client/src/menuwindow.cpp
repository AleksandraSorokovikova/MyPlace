...
#include "include/searchaccounts.h"
...
MenuWindow::MenuWindow(QWidget *parent, QString id, QString nickname_)
    : QMainWindow(parent), ui(new Ui::MenuWindow), user_id(id),
      nickname(nickname_) {
  ui->setupUi(this);
  menu_icons icons;
  ...
  if (type == typeListWidget::LABELS) {
    if (Client::update_label_list(labelList, user_id) == NO_CONNECTION) {
      ...
      for (const auto &x : labelList.id_list) {
        ...
        QListWidgetItem *item =
            new QListWidgetItem(QString::fromStdString(label.name));
        model->insertMarker(QGeoCoordinate(std::stod(label.longitude),
                                           std::stod(label.latitude)));
        model->labels_on_coordinate1[std::make_pair(
            std::stod(label.latitude), std::stod(label.longitude))] = label;
    ...
    if (Client::update_subscribes(user_id, users) == NO_CONNECTION) {
      ...
      for (const auto &user : users) {
        QListWidgetItem *item =
            new QListWidgetItem(QString::fromStdString(user));
        ui->listWidget->addItem(item);
      }
    }
  }
...
void MenuWindow::on_listWidget_itemDoubleClicked(QListWidgetItem *item) {
  if (type == typeListWidget::LABELS) {
    OpenLabel open(
        labelList.get_by_id(item->data(12).toString().toStdString()));
    open.setModal(true);
    open.exec();
  } else {
...
void MenuWindow::on_update_clicked() { update(); }
...
void MenuWindow::on_search_returnPressed() { on_search_button_pressed(); }
...
void MenuWindow::on_logout_clicked() {
  if (QMessageBox::Yes ==
      QMessageBox::question(this, "Close Confirmation", "Exit?",
