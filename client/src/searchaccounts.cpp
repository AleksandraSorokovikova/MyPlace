...
SearchAccounts::SearchAccounts(QWidget *parent, QString id, QString subscribe_,
                               bool is_serach_)
    : QDialog(parent), ui(new Ui::SearchAccounts), user_id(id),
      subscribe_name(subscribe_), is_search(is_serach_) {
  ui->setupUi(this);
  ...
  int return_code =
      Client::user_information(subscribe_name, lables_size, subscribes_size);
  switch (return_code) {
  case NO_CONNECTION:
...
SearchAccounts::~SearchAccounts() { delete ui; }
...
void SearchAccounts::on_cancel_clicked() { hide(); }
