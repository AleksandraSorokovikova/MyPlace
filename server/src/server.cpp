...
void add_label(stream_ptr stream) {
  ...
  Label label(name, nickname, type, description, address, latitude, longitude,
              labelList);
...
void subscribe(stream_ptr stream) {
  ...
  if (!userList.nickname_in_list(nickname) || nickname == other_nickname ||
      userList.get_user_by_nickname(nickname).is_subscribed(other_nickname)) {
...
void server(io_service &io_service_) {
  ip::tcp::acceptor acceptor_(io_service_,
                              ip::tcp::endpoint(ip::tcp::v4(), 8010));
