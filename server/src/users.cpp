...
[[nodiscard]] bool
User_List::nickname_in_list(const std::string &nickname) const {
...
void User_List::add(std::string nickname_, std::string password_) {
  ...
  data.emplace(std::piecewise_construct, std::forward_as_tuple(nickname_),
               std::forward_as_tuple(nickname_, password_, id_));
  nickname_list.insert(nickname_);
  id_list.insert(id_);
}
[[nodiscard]] bool
User_List::check_password(const std::string &nickname,
                          const std::string &password) const {
  return data.find(nickname)->second.password == password;
}

void User_List::add_label_for_current_user(const std::string &nickname,
...
void User_List::subscribe_user(const std::string &nickname,
...
void Active_Users::deactivate(const std::string &id) {
  ...
  auto id_user = id_list.find(id);
  id_list.erase(id_user);
