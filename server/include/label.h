...
struct Label {
  ...
  Label(std::string name_, std::string nickname_, std::string type_,
        std::string description_, std::string address_, std::string latitude_,
        std::string longitude_, const Label_List &label_list)
      : name(std::move(name_)), nickname(std::move(nickname_)),
        type(std::move(type_)), description(std::move(description_)),
        address(std::move(address_)), latitude(std::move(latitude_)),
        longitude(std::move(longitude_)) {
    id = create_id(label_list);
  }

  Label(std::string id_, std::string name_, std::string nickname_,
        std::string type_, std::string description_, std::string address_,
        std::string latitude_, std::string longitude_)
      : id(std::move(id_)), name(std::move(name_)),
        nickname(std::move(nickname_)), type(std::move(type_)),
